...
import { Plus, PackagePlus, X, RefreshCw, Download, ChevronLeft, ChevronRight, Search } from 'lucide-react'
import toast from 'react-hot-toast'
import { listarEntradas, registrarEntrada } from '../services/movimientosApi'
import { listarStock } from '../services/inventarioApi'
import { listarDestinos, listarTiposMovimiento, idParaEntrada, idPreferidoParaEntrada } from '../services/catalogosApi'
import { mapaNombresParaMostrarPorId } from '../services/authApi'
import { descargarCsv, formatearFechahora } from '../utils/csv'

const PAGE_SIZE = 10

const SKEL_W = ['60%', '40%', '75%', '35%']
...
export default function Entrada() {
  ...
  async function cargar() {
    setLoading(true)
    const [lista, mapa] = await Promise.all([listarEntradas(), mapaNombresParaMostrarPorId()])
    setEntradas(lista); setNombres(mapa); setLoading(false)
  }

  useEffect(() => { cargar() }, [])
  useEffect(() => { setPage(1) }, [busqueda])

  function getNombre(mov) {
  ...
  const gruposFiltrados = grupos.filter(g => {
    ...
    return g[0].nombreDestino?.toLowerCase().includes(q) ||
      ...
      getNombre(g[0]).toLowerCase().includes(q)
  })
  ...
  function exportar() {
    ...
    const ord = [...entradas].sort((a, b) => new Date(b.fecha) - new Date(a.fecha))
    const filas = [
      ['Fecha y hora', 'Destino', 'Producto', 'Cantidad', 'Ingresado por'],
      ...ord.map(m => [formatearFechahora(m.fecha), m.nombreDestino, m.nombreProducto || '', m.cantidad, getNombre(m)]),
    ]
    const hoy = new Date()
    descargarCsv(filas, `entradas_${hoy.getFullYear()}-${String(hoy.getMonth()+1).padStart(2,'0')}-${String(hoy.getDate()).padStart(2,'0')}.csv`)
  }

  return (
    <div className="flex flex-col h-full p-4 md:p-7 gap-4">
      <div>
        <h2 className="text-xl md:text-2xl font-bold text-slate-800 dark:text-white">Entrada</h2>
        <p className="text-slate-500 dark:text-white/60 text-xs mt-0.5">Registrar una entrada de stock</p>
      </div>
        ...
        {!loading && gruposFiltrados.length === 0 ? (
          <div className="flex-1 flex flex-col items-center justify-center gap-3 text-slate-400 dark:text-white/40 py-12">
            <div className="w-16 h-16 rounded-2xl bg-slate-100 dark:bg-white/5 flex items-center justify-center">
              <PackagePlus size={32} strokeWidth={1.3} />
            </div>
            <div className="text-center">
              <p className="text-sm font-medium">{busqueda ? 'Sin resultados para esa búsqueda' : 'Sin entradas registradas'}</p>
              <p className="text-xs mt-0.5 text-slate-300 dark:text-white/20">
                {busqueda ? 'Probá con otro término' : 'Registrá la primera entrada con el botón Agregar'}
              </p>
            </div>
          </div>
        ) : (
              ...
              <table className="w-full text-sm text-left" style={{ minWidth: 480 }}>
                <thead className="text-slate-500 dark:text-white/50 text-xs uppercase border-b border-slate-200 dark:border-white/10 sticky top-0 bg-white dark:bg-[#1A2332]">
                  <tr>
                    ...
                    <th className="px-4 py-3 font-semibold">Ingresado por</th>
                  </tr>
                </thead>
                <tbody>
                  {loading ? <SkeletonRows cols={4} /> : gruposPagina.map((grupo, i) => {
                    ...
                    return (
                      <tr key={i} className="border-b border-slate-100 dark:border-white/5 hover:bg-slate-50 dark:hover:bg-white/10 transition-colors">
                        ...
                        <td className="px-4 py-3 text-slate-400 dark:text-white/50 text-xs italic">{getNombre(primera) || '—'}</td>
                      </tr>
                    )
                  })}
                </tbody>
      ...
      {modal && (
        <ModalEntrada onClose={() => setModal(false)} onExito={() => { setModal(false); cargar() }} />
      )}
    </div>
  )
}

function ModalEntrada({ onClose, onExito }) {
  const [stock,     setStock]     = useState([])
  const [destinos,  setDestinos]  = useState([])
  const [idTipoEnt, setIdTipoEnt] = useState(null)
  ...
  useEffect(() => {
    async function init() {
      ...
      setStock(rawStock); setDestinos(d); setIdTipoEnt(idParaEntrada(tipos))
      const idDest = idPreferidoParaEntrada(d)
      setDestino(idDest ? String(idDest) : d[0]?.id ? String(d[0].id) : '')
  ...
  async function guardar() {
    ...
    for (const fila of filas) {
      const res = await registrarEntrada({ idProducto: Number(fila.idProducto), cantidad: Number(fila.cantidad), idTipoMov: idTipoEnt, idDestino: Number(destino) })
      if (!res.ok) { setError(res.message); setGuardando(false); return }
    }
    setGuardando(false)
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4">
      <div className="bg-white dark:bg-[#1A2332] rounded-xl w-full max-w-md shadow-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-5 border-b border-gray-100 dark:border-white/5">
          <h3 className="font-bold text-gray-800 dark:text-white">Registrar entrada</h3>
        ...
        <div className="p-5 flex flex-col gap-4">
          {cargando ? (
            ...
            <>
              <div>
                <label className="text-xs font-semibold text-gray-500 dark:text-slate-400 uppercase tracking-wide mb-1 block">Destino</label>
                ...
                {filas.map((fila, i) => (
                  <div key={i} className="flex gap-2 mb-2">
                    <select value={fila.idProducto}
                      onChange={e => { const f = [...filas]; f[i].idProducto = e.target.value; setFilas(f) }}
                      ...
                      {stock.map(s => <option key={s.idProducto} value={s.idProducto}>{s.nombreProducto}</option>)}
                    </select>
                    <input type="number" placeholder="Cant." value={fila.cantidad} min={1}
                      onChange={e => { const f = [...filas]; f[i].cantidad = e.target.value; setFilas(f) }}
              ...
              {error && <p className="text-red-500 text-sm bg-red-50 dark:bg-red-500/10 border border-red-200 dark:border-red-500/20 rounded px-3 py-2">{error}</p>}
